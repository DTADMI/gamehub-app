...
test.describe('Systems Discovery — BOD Signal & Defend', () => {
    ...
    test('Signal pack via deep-link: Intro → BSD1→BSD3 → Wrap; meter steady', async ({page}) => {
        await page.goto('/games/systems-discovery?pack=signal');
        ...
        await expect(page.getByText(/Signal & Defend badge/i)).toBeVisible();
    });
});
