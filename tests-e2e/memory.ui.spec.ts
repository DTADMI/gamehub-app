...
test.describe("Memory UI interactions", () => {
    test("matched cards become invisible but preserve layout space", async ({
                                                                                page,
                                                                            }) => {
        ...
        const cards = page.locator(
            '[data-testid="memory-card"], div[style*="transform-style"]',
        );
        ...
        for (let i = 0; i < 18 && !found; i += 2) {
            ...
            if (
                ...
                if (
                    (class1 || "").includes("opacity-0") &&
                    (class2 || "").includes("opacity-0")
                ) {
                    found = true;
                    break;
                }
