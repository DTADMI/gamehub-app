...
function SettingsConsumer() {
  ...
  return (
      <div>
        ...
        <button onClick={() => setAuthState(true, false)} aria-label="auth-on">
          auth
        </button>
        <button onClick={() => setAuthState(true, true)} aria-label="sub-on">
          sub
        </button>
      </div>
  );
...
describe("GameSettingsProvider — particles & mode gating", () => {
  ...
  it("persists mode/auth/subscriber flags to localStorage", () => {
    ...
    // Get the stored value directly from our mock
